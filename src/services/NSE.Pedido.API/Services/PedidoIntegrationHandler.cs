...
namespace NSE.Pedido.API.Services
{
    public class PedidoIntegrationHandler : BackgroundService
    {
        ...
        {
            _bus.SubscribeAsync<PedidoCanceladoIntegrationEvent>("PedidoCancelado", CancelarPedido);

            _bus.SubscribeAsync<PedidoPagoIntegrationEvent>("PedidoPago", FinalizarPedido);
        }
