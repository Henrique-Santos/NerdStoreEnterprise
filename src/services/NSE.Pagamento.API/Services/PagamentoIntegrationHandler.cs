...
namespace NSE.Pagamento.API.Services
{
    public class PagamentoIntegrationHandler : BackgroundService
    {
        ...
        {
            _bus.RespondAsync<PedidoIniciadoIntegrationEvent, ResponseMessage>(AutorizarPagamento);
        }

        private void SetSubscribers()
        {
            _bus.SubscribeAsync<PedidoCanceladoIntegrationEvent>("PedidoCancelado", CancelarPagamento);

            _bus.SubscribeAsync<PedidoBaixadoEstoqueIntegrationEvent>("PedidoBaixadoEstoque", CapturarPagamento);
        }
